...
func c_ipcount_linux() (datapoint.MultiDataPoint, error) {
	...
	Add(&md, "linux.net.ip_count", v4c, datapoint.TagSet{"version": "4"}, metadata.Gauge, "IP_Addresses", "")
	Add(&md, "linux.net.ip_count", v6c, datapoint.TagSet{"version": "6"}, metadata.Gauge, "IP_Addresses", "")
	return md, nil
}

func c_ifstat_linux() (datapoint.MultiDataPoint, error) {
	var md datapoint.MultiDataPoint
	...
	err := readLine("/proc/net/dev", func(s string) error {
		...
		tags := datapoint.TagSet{"iface": intf}
		...
		for i, v := range stats {
			Add(&md, "linux.net."+bond_string+strings.Replace(netFields[i].key, ".", "_", -1), v, datapoint.TagSet{
				"iface":     intf,
				"direction": direction(i),
			}, netFields[i].rate, netFields[i].unit, "")
			if i < 4 || (i >= 8 && i < 12) {
				Add(&md, "os.net."+bond_string+strings.Replace(netFields[i].key, ".", "_", -1), v, datapoint.TagSet{
					"iface":     intf,
					"direction": direction(i),
				}, netFields[i].rate, netFields[i].unit, "")
