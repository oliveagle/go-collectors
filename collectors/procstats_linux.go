...
func c_procstats_linux() (datapoint.MultiDataPoint, error) {
	...
	if err := readLine("/proc/vmstat", func(s string) error {
		...
		case "pgpgin", "pgpgout", "pswpin", "pswpout", "pgfault", "pgmajfault":
			...
			if mio != nil {
				Add(&md, "linux.mem."+mio[1], m[2], datapoint.TagSet{"direction": mio[2]}, metadata.Counter, metadata.Page, "")
			} else {
	...
	if err := readLine("/proc/interrupts", func(s string) error {
		...
		for i, val := range cols[1:] {
			...
			Add(&md, "linux.interrupts", val, datapoint.TagSet{"type": irq_type, "cpu": strconv.Itoa(i)}, metadata.Gauge, metadata.Interupt, irq_type_desc[irq_type])
		}
		return nil
	}); err != nil {
		Error = err
	...
	for _, fi := range bondDevices {
		...
		if err := readLine(bondingPath+"/"+fi.Name(), func(s string) error {
			...
			if f[0] == "MII Status" && iface != "" {
				...
				Add(&md, "linux.net.bond.slave.is_up", status, datapoint.TagSet{"slave": iface, "bond": fi.Name()}, metadata.Gauge, metadata.Bool, "The status of a bond interface.")
			}
			return nil
		}); err != nil {
			Error = err
