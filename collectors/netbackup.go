...
func c_netbackup_jobs() (datapoint.MultiDataPoint, error) {
	...
	for _, r := range latest {
		tags := datapoint.TagSet{"class": r.Class, "client": r.Client, "schedule": r.Schedule}
...
func c_netbackup_frequency() (datapoint.MultiDataPoint, error) {
	var md datapoint.MultiDataPoint
		...
		if strings.HasPrefix(strings.TrimSpace(line), "Frequency:") {
			...
			if len(f) == 5 {
				...
				for _, client := range clients {
					tags := datapoint.TagSet{"class": class, "client": client, "schedule": schedule}
