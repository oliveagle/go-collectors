...
func c_iis_webservice() (datapoint.MultiDataPoint, error) {
	...
	for _, v := range dst {
		Add(&md, "iis.bytes", v.BytesReceivedPersec, datapoint.TagSet{"site": v.Name, "direction": "received"}, metadata.Counter, metadata.BytesPerSecond, descIISBytesReceivedPersec)
		Add(&md, "iis.bytes", v.BytesSentPersec, datapoint.TagSet{"site": v.Name, "direction": "sent"}, metadata.Counter, metadata.BytesPerSecond, descIISBytesSentPersec)
		Add(&md, "iis.requests", v.CGIRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "cgi"}, metadata.Counter, metadata.PerSecond, descIISCGIRequestsPersec)
		Add(&md, "iis.connection_attempts", v.ConnectionAttemptsPersec, datapoint.TagSet{"site": v.Name}, metadata.Counter, metadata.PerSecond, descIISConnectionAttemptsPersec)
		Add(&md, "iis.requests", v.CopyRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "copy"}, metadata.Counter, metadata.PerSecond, descIISCopyRequestsPersec)
		Add(&md, "iis.connections", v.CurrentConnections, datapoint.TagSet{"site": v.Name}, metadata.Gauge, metadata.Count, descIISCurrentConnections)
		Add(&md, "iis.requests", v.DeleteRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "delete"}, metadata.Counter, metadata.PerSecond, descIISDeleteRequestsPersec)
		Add(&md, "iis.requests", v.GetRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "get"}, metadata.Counter, metadata.PerSecond, descIISGetRequestsPersec)
		Add(&md, "iis.requests", v.HeadRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "head"}, metadata.Counter, metadata.PerSecond, descIISHeadRequestsPersec)
		Add(&md, "iis.requests", v.ISAPIExtensionRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "isapi"}, metadata.Counter, metadata.PerSecond, descIISISAPIExtensionRequestsPersec)
		Add(&md, "iis.errors", v.LockedErrorsPersec, datapoint.TagSet{"site": v.Name, "type": "locked"}, metadata.Counter, metadata.PerSecond, descIISLockedErrorsPersec)
		Add(&md, "iis.requests", v.LockRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "lock"}, metadata.Counter, metadata.PerSecond, descIISLockRequestsPersec)
		Add(&md, "iis.requests", v.MkcolRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "mkcol"}, metadata.Counter, metadata.PerSecond, descIISMkcolRequestsPersec)
		Add(&md, "iis.requests", v.MoveRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "move"}, metadata.Counter, metadata.PerSecond, descIISMoveRequestsPersec)
		Add(&md, "iis.errors", v.NotFoundErrorsPersec, datapoint.TagSet{"site": v.Name, "type": "notfound"}, metadata.Counter, metadata.PerSecond, descIISNotFoundErrorsPersec)
		Add(&md, "iis.requests", v.OptionsRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "options"}, metadata.Counter, metadata.PerSecond, descIISOptionsRequestsPersec)
		Add(&md, "iis.requests", v.PostRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "post"}, metadata.Counter, metadata.PerSecond, descIISPostRequestsPersec)
		Add(&md, "iis.requests", v.PropfindRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "propfind"}, metadata.Counter, metadata.PerSecond, descIISPropfindRequestsPersec)
		Add(&md, "iis.requests", v.ProppatchRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "proppatch"}, metadata.Counter, metadata.PerSecond, descIISProppatchRequestsPersec)
		Add(&md, "iis.requests", v.PutRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "put"}, metadata.Counter, metadata.PerSecond, descIISPutRequestsPersec)
		Add(&md, "iis.requests", v.SearchRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "search"}, metadata.Counter, metadata.PerSecond, descIISSearchRequestsPersec)
		Add(&md, "iis.requests", v.TraceRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "trace"}, metadata.Counter, metadata.PerSecond, descIISTraceRequestsPersec)
		Add(&md, "iis.requests", v.UnlockRequestsPersec, datapoint.TagSet{"site": v.Name, "method": "unlock"}, metadata.Counter, metadata.PerSecond, descIISUnlockRequestsPersec)
	}
	return md, nil
}
...
func c_iis_apppool() (datapoint.MultiDataPoint, error) {
	...
	var md datapoint.MultiDataPoint
	for _, v := range dst {
		tags := datapoint.TagSet{"name": v.Name}
		...
		Add(&md, "iis.apppool.processes", v.CurrentWorkerProcesses, datapoint.TagSet{"name": v.Name, "type": "current"}, metadata.Gauge, metadata.Count, descIISAppPoolCurrentWorkerProcesses)
		Add(&md, "iis.apppool.processes", v.MaximumWorkerProcesses, datapoint.TagSet{"name": v.Name, "type": "maximum"}, metadata.Gauge, metadata.Count, descIISAppPoolMaximumWorkerProcesses)
		Add(&md, "iis.apppool.processes", v.RecentWorkerProcessFailures, datapoint.TagSet{"name": v.Name, "type": "failed"}, metadata.Gauge, metadata.Count, descIISAppPoolRecentWorkerProcessFailures)
		Add(&md, "iis.apppool.events", v.TotalApplicationPoolRecycles, datapoint.TagSet{"name": v.Name, "type": "recycled"}, metadata.Counter, metadata.Event, descIISAppPoolTotalApplicationPoolRecycles)
		Add(&md, "iis.apppool.events", v.TotalWorkerProcessesCreated, datapoint.TagSet{"name": v.Name, "type": "created"}, metadata.Counter, metadata.Event, descIISAppPoolTotalWorkerProcessesCreated)
		Add(&md, "iis.apppool.events", v.TotalWorkerProcessFailures, datapoint.TagSet{"name": v.Name, "type": "failed_crash"}, metadata.Counter, metadata.Event, descIISAppPoolTotalWorkerProcessFailures)
		Add(&md, "iis.apppool.events", v.TotalWorkerProcessPingFailures, datapoint.TagSet{"name": v.Name, "type": "failed_ping"}, metadata.Counter, metadata.Event, descIISAppPoolTotalWorkerProcessPingFailures)
		Add(&md, "iis.apppool.events", v.TotalWorkerProcessShutdownFailures, datapoint.TagSet{"name": v.Name, "type": "failed_shutdown"}, metadata.Counter, metadata.Event, descIISAppPoolTotalWorkerProcessShutdownFailures)
		Add(&md, "iis.apppool.events", v.TotalWorkerProcessStartupFailures, datapoint.TagSet{"name": v.Name, "type": "failed_startup"}, metadata.Counter, metadata.Event, descIISAppPoolTotalWorkerProcessStartupFailures)
	}
	return md, nil
}
