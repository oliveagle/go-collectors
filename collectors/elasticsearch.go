...
func c_elasticsearch() (datapoint.MultiDataPoint, error) {
	var status esStatus
	...
	add := func(name string, val interface{}, ts datapoint.TagSet) {
		tags := datapoint.TagSet{"cluster": stats.ClusterName}
		for k, v := range ts {
			tags[k] = v
		}
...
func c_elasticsearch_indices() (datapoint.MultiDataPoint, error) {
	...
	for k, v := range health.Indices {
		ts := datapoint.TagSet{"index_name": k, "cluster": cluster}
